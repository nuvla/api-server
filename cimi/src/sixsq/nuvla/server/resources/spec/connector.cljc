(ns sixsq.nuvla.server.resources.spec.connector)

;;
;; Note that all of the keys and keys specs are already defined
;; with the connector template.  This file exists only to allow a
;; place to define new keys, if that should become necessary.
;;
;; As for the connector template, this is a "base class" so there
