(ns sixsq.nuvla.server.middleware.authn-info-test
  (:require
    ...
    [sixsq.nuvla.server.middleware.authn-info :refer :all]))
...
(deftest check-extract-authn-info
  (are [expected header]
    (= expected (extract-header-authn-info {:headers {authn-info-header header}}))
    nil nil
    nil ""
...
(deftest check-claims->authn-info
  (are [expected claims]
    (= expected (cookie-info->authn-info claims))
    nil nil
    nil {}
  ...
  (let [handler  (wrap-authn-info identity)
    ...
    (are [expected request] (= expected (auth/current-authentication (handler request)))
                            ...
                            {:claims  #{"group/nuvla-anon", "user/uname"}
                             :user-id "user/uname"} {:headers {authn-info-header "user/uname"}}

                            {:claims  #{"group/r1", "group/nuvla-anon", "user/uname"}
                             :user-id "user/uname"} {:headers {authn-info-header "user/uname group/r1"}}

                            {:claims  #{"group/r1", "group/r2", "group/nuvla-anon", "user/uname"}
                             :user-id "user/uname"} {:headers {authn-info-header "user/uname group/r1 group/r2"}}

                            {:claims  #{"group/nuvla-anon", "user/uname2"}
                             :user-id "user/uname2"} {:cookies {authn-cookie cookie-id}}

                            {:claims  #{"group/nuvla-user", "group/alpha-role",
                                        session-a, "group/nuvla-anon", "user/uname2"}
                             :user-id "user/uname2"} {:cookies {authn-cookie cookie-id-roles}}
