...
(deftest search-event
  (doseq [event-type ["resource.create" "resource.created"]
  ...
  (is (= 3 (count (t/search-event "user/1" {:event-type "resource.create"}))))
  (is (= 3 (count (t/search-event "user/1" {:event-type "resource.created"}))))
