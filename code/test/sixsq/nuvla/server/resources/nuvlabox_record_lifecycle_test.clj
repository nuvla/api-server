(ns sixsq.nuvla.server.resources.nuvlabox-record-lifecycle-test
  (:require
    ...
    [sixsq.nuvla.server.resources.nuvlabox.utils-test :as nbtu]
...
(deftest lifecycle
  (with-redefs [utils/list-all-identifiers-for-series (fn [_] (map #(str "my-identifier-" %) (range 10)))]
    (let [session (-> (ltu/ring-app)
      ...
      ;; Non admin users should not create
      ;; FIXME: relies on quotas and creating these disturbs later tests
      #_(with-redefs [utils/call-vpn-api (fn [_ _ _ _] [201 sample-response-map])]
          (-> session-jane
              (request base-uri
                       :request-method :post
                       :body (json/write-str valid-nuvlabox))
              (ltu/body->edn)
              (ltu/is-status 403))                          ;; only allowed to create nanos

          ;; no quota defined to create nano
          (-> session-jane
              (request base-uri
                       :request-method :post
      ...
      ;; creating  as a normal user without any quota should not succeed
      ;; FIXME: These tests rely on quotas.
      #_(with-redefs [utils/call-vpn-api (fn [_ _ _ _] [201 sample-response-map])]
          (-> session-jane
              (request base-uri
                       :request-method :post
              ...
              (ltu/body->edn)
              (ltu/is-status 403))                          ;; In any case, a simple user could only create nanos
          (-> session-jane

              (request base-uri
                       :request-method :post
                       :body (json/write-str (assoc valid-nano
                                               :sslKey "kkk"
                                               :sslCA "ca"
                                               :sslCert "cert"
                                               :macAddress "00:00:00:00:00:00")))
              (ltu/body->edn)
              (ltu/is-status 412)))                         ;; missing quota
      ...
      ;;Without any identifiers to choose from, a nano record will fail schema validation
      ;;whereas a regular nuvlabox will not
      (with-redefs [utils/list-all-identifiers-for-series (fn [_] ()) ;;no identifiers to choose from
                    utils/call-vpn-api (fn [_ _ _ _] [201 sample-response-map])]
      ...
      ;;When every possible identifiers are already taken,  the nano record will fail schema validation
      ;; standard nuvlabox should not be impacted
      ;; FIXME: Depends on quotas.
      #_(with-redefs [utils/already-used-identifiers (fn [] (map #(str "my-identifier-" %) (range 10)))
                    utils/call-vpn-api (fn [_ _ _ _] [201 sample-response-map])]
        ...
        (-> session-jane
            (request base-uri
                     ...
                     :body (json/write-str valid-nano))
            (ltu/body->edn)
            (ltu/is-status 412)))                           ;; user has no quota yet


      ;; With sufficient quota should succeed
      (with-redefs [utils/call-vpn-api (fn [_ _ _ _] [201 sample-response-map])]
        (-> session-jane
            (request base-uri
                     :request-method :post
      ...
      (with-redefs [utils/call-vpn-api (fn [_ _ _ _] [201 sample-response-map])]
        (let [nb1 (random-nano "owner1")
          ...
          ;;same owners, different macAdresses : quota should have reached the limit
          ;; FIXME: Depends on quotas.
          #_(doseq [nb #{(assoc nb1 :macAddress (random-mac-address)) (assoc nb2 :macAddress (random-mac-address))}]
            (-> session-jane
                (request base-uri
                         :request-method :post
                         :body (json/write-str nb))
