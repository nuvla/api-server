(ns sixsq.nuvla.server.resources.spec.user-template-password-test
  (:require
    [clojure.test :refer [deftest is]]
    [sixsq.nuvla.server.resources.spec.spec-test-utils :as stu]
    [sixsq.nuvla.server.resources.spec.user-template-password :as ut-password]
    [sixsq.nuvla.server.resources.user-template :as user-tpl]
    [sixsq.nuvla.server.resources.user-template-password :as user-tpl-password]))


(def valid-acl {:owner {:principal "ADMIN"
                        :type      "ROLE"}
                :rules [{:type      "ROLE"
                         :principal "ADMIN"
                         :right     "ALL"}]})


(deftest check-user-template-password-schema
  (let [timestamp "1964-08-25T10:00:00.0Z"
        tpl {:id                (str user-tpl/resource-type "/" user-tpl-password/registration-method)
             :resource-type     user-tpl/resource-type
             :name              "my-template"
             :description       "my template"
             :group             "my group"
             :tags              #{"1", "2"}
             :created           timestamp
             :updated           timestamp
             :acl               valid-acl

             :method            user-tpl-password/registration-method
             :instance          user-tpl-password/registration-method

             :password          "plaintext-password"
             :password-repeated "plaintext-password"
             :email             "someone@example.org"}

        create-tpl {:name          "my-create"
                    :description   "my create description"
                    :tags          #{"3", "4"}
                    :resource-type user-tpl/collection-type
                    :template      (dissoc tpl :id)}]

    ;; check the registration schema (without href)
    (stu/is-valid ::ut-password/schema tpl)

    ;; mandatory attributes
    (doseq [attr #{:id :resource-type :created :updated :acl :method}]
      (stu/is-invalid ::ut-password/schema (dissoc tpl attr)))

    (doseq [attr #{:name :description :tags :password :password-repeated :email}]
      (stu/is-valid ::ut-password/schema (dissoc tpl attr)))

    ;; check the create template schema (with href)
    (stu/is-valid ::ut-password/schema-create create-tpl)
    (stu/is-valid ::ut-password/schema-create (assoc-in create-tpl [:template :href] "user-template/abc"))
    (stu/is-invalid ::ut-password/schema-create (assoc-in create-tpl [:template :href] "bad-reference/abc"))

    ;; mandatory attributes
    (doseq [attr #{:resource-type :template}]
      (stu/is-invalid ::ut-password/schema-create (dissoc create-tpl attr)))

    ;; mandatory template attributes
    (doseq [attr #{:password :password-repeated :email}]
      (let [create-tpl (assoc create-tpl :template (dissoc tpl attr))]
        (stu/is-invalid ::ut-password/schema-create create-tpl)))

    ;; optional attributes
    (doseq [attr #{:name :description :tags}]
      (stu/is-valid ::ut-password/schema-create (dissoc create-tpl attr)))))
