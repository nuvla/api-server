(ns com.sixsq.nuvla.server.resources.mec.mm3-client-test
  (:require
...
(deftest test-check-health-success
  (testing "Successful health check"
    (with-redefs [http/get (fn [_url _opts] mock-health-success)]
      (let [result (mm3/check-health "https://mepm.example.com:8443" {:retry-attempts 1})]
        (is (:success? result))
...
(deftest test-query-capabilities-success
  (testing "Successful capabilities query"
    (with-redefs [http/get (fn [_url _opts] mock-capabilities-success)]
      (let [result (mm3/query-capabilities "https://mepm.example.com:8443" {:retry-attempts 1})]
        (is (:success? result))
        (is (= 200 (:status result)))
...
(deftest test-query-resources-success
  (testing "Successful resources query"
    (with-redefs [http/get (fn [_url _opts] mock-resources-success)]
      (let [result (mm3/query-resources "https://mepm.example.com:8443" {:retry-attempts 1})]
        (is (:success? result))
        (is (= 200 (:status result)))
  ...
  (testing "Successful platform configuration"
    (with-redefs [http/post (fn [_url _opts] {:status 200 :body {:status "configured"}})]
      (let [config {:service-registry true :traffic-rules false}
            result (mm3/configure-platform "https://mepm.example.com:8443" config {:retry-attempts 1})]
        (is (:success? result))
        (is (= 200 (:status result)))))))
    ...
    (with-redefs [http/get (fn [_url _opts] 
                             {:status 200 
                              :body {:name "MEPM-001"
                                     ...
                                     :status "ONLINE"}})]
      (let [result (mm3/get-platform-info "https://mepm.example.com:8443" {:retry-attempts 1})]
        (is (:success? result))
    ...
    (let [call-count (atom 0)]
      (with-redefs [http/get (fn [_url _opts]
                               ...
                               (if (< @call-count 3)
                                 ...
                                 mock-health-success))]
        (let [result (mm3/check-health "https://mepm.example.com:8443" {:retry-attempts 3})]
          (is (:success? result))
...
(deftest test-healthy-predicate
  (testing "healthy? convenience function"
    (with-redefs [http/get (fn [_url _opts] mock-health-success)]
      (is (true? (mm3/healthy? "https://mepm.example.com:8443" {:retry-attempts 1}))))
    
    (with-redefs [http/get (fn [_url _opts] mock-health-failure)]
      (is (false? (mm3/healthy? "https://mepm.example.com:8443" {:retry-attempts 1}))))))


(deftest test-get-capabilities-convenience
  (testing "get-capabilities convenience function returns data or nil"
    (with-redefs [http/get (fn [_url _opts] mock-capabilities-success)]
      (let [caps (mm3/get-capabilities "https://mepm.example.com:8443" {:retry-attempts 1})]
        (is (some? caps))
        (is (= ["x86_64" "arm64"] (:platforms caps)))))
    
    (with-redefs [http/get (fn [_url _opts] {:status 500 :body {}})]
      (is (nil? (mm3/get-capabilities "https://mepm.example.com:8443" {:retry-attempts 1}))))))


(deftest test-get-resources-convenience
  (testing "get-resources convenience function returns data or nil"
    (with-redefs [http/get (fn [_url _opts] mock-resources-success)]
      (let [resources (mm3/get-resources "https://mepm.example.com:8443" {:retry-attempts 1})]
        (is (some? resources))
        (is (= 64 (:cpu-cores resources)))))
    
    (with-redefs [http/get (fn [_url _opts] {:status 500 :body {}})]
      (is (nil? (mm3/get-resources "https://mepm.example.com:8443" {:retry-attempts 1}))))))
  ...
  (testing "URLs are correctly constructed"
    (let [captured-urls (atom [])]
      (with-redefs [http/get (fn [url _opts] 
                               ...
                               mock-health-success)]
        (mm3/check-health "https://mepm.example.com:8443" {:retry-attempts 1})
        (mm3/query-capabilities "https://mepm.example.com:8443" {:retry-attempts 1})
        (mm3/query-resources "https://mepm.example.com:8443" {:retry-attempts 1})
        (mm3/get-platform-info "https://mepm.example.com:8443" {:retry-attempts 1})
        
        (is (= "https://mepm.example.com:8443/mm3/health" (first @captured-urls)))
        (is (= "https://mepm.example.com:8443/mm3/capabilities" (second @captured-urls)))
        (is (= "https://mepm.example.com:8443/mm3/resources" (nth @captured-urls 2)))
        (is (= "https://mepm.example.com:8443/mm3/platform-info" (nth @captured-urls 3)))))))
  ...
  (testing "HTTP options are properly configured"
    (let [captured-opts (atom nil)]
      (with-redefs [http/get (fn [_url opts] 
                               ...
                               mock-health-success)]
        (mm3/check-health "https://mepm.example.com:8443" 
                          {:timeout 60000
