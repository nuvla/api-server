(ns com.sixsq.nuvla.server.resources.credential-ssh-key-lifecycle-test
  (:require
    ...
    [com.sixsq.nuvla.server.resources.credential-template-ssh-key :as ct-ssh-key]
...
(deftest lifecycle
  ...
  (let [session          (-> (ltu/ring-app)
    ...
    (let [resp    (-> session-user
                    (request base-uri
                      :request-method :post
                      :body (j/write-value-as-string upload-pvtkey))
                    (ltu/body->edn)
                    (ltu/is-status 201))
          id      (ltu/body-resource-id resp)
          keypair (get-in resp [:response :body])
