(ns com.sixsq.nuvla.server.resources.common.dynamic-load
...
(defn resource?
  "If the given symbol represents a resource namespace, the symbol
   ...
   'com.sixsq.nuvla.server.resources.' and do not contain the
   string 'test'."
  [sym]
  (let [ns-name (name sym)]
    (and
      (re-matches #"^com\.sixsq\.nuvla\.server\.resources\.[\w-]+$" ns-name)
