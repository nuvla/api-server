...
"
  (:require
    ...
    [sixsq.nuvla.server.resources.group :as g]
...
(defn create-user
  ...
  (std-crud/add-if-absent
    (str user/resource-type " '" email "'") user/resource-type
    {:template
...
(defn add-to-group
  ...
  (let [{:keys [users]} (crud/retrieve-by-id-as-admin group-id)
        {:keys [status body]} (g/edit-impl
                                {:params      {:uuid          (u/id->uuid group-id)
                                               :resource-name g/resource-type}
...
(defmethod callback/execute action-name
  [{callback-id                               :id
    {existing-user-id :user-id, email :email} :data
