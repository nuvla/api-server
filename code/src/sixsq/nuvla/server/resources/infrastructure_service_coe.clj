    ...
    (let [resource-id (:id resource)
      ...
      (if (= 201 status)
        (let [job-msg (format "created job %s with id %s" job-name job-id)]
          ...
          ;; legacy events
          #_(infra-service/event-state-change resource (assoc-in request [:body :state] new-state))
          #_(event-utils/create-event resource-id job-msg (a/default-acl (auth/current-authentication request)))
  ...
  (try
    ...
    (let [id           (:id service)
      ...
      (-> id
          ...
          (db/edit request))
      ;; Legacy events
      #_(event-utils/create-event id "STARTING" (a/default-acl (auth/current-authentication request))
                                  :severity "low"
                                  :category "state")
