...
(s/def ::data-filters (su/only-keys :req-un [::filter ::time-start ::time-end ::data-type]))
(s/def ::filters (s/coll-of ::data-filters))

(s/def ::records (su/only-keys :opt-un [::records-ids ::filters]))
(s/def ::objects (su/only-keys :opt-un [::objects-ids ::filters]))

(s/def ::data (su/only-keys-maps {:opt-un [::records ::objects]}))
